...
export async function generateMetadata({ params }: MoviePageProps): Promise<Metadata> {
    const movie = movieData.find((m) => m.slug === params.id);
...
export default function MoviePage({ params }: MoviePageProps) {
    // Find movie by slug from static data
    const movie = movieData.find((m) => m.slug === params.id);
    ...
    // If it's a YouTube embed URL (short/watch), we use iframe. Else we assume direct video file.
    // Our util returns null if not youtube.
    // But we also want to support direct embed if provided directly.
    const isEmbed = !!youtubeEmbedUrl || movie.videoUrl.includes('vimeo.com') || movie.videoUrl.includes('youtube.com') || movie.videoUrl.includes('youtu.be');

    // For autoplaying (optional) - let's append params
    ...
    return (
        <div className="min-h-screen bg-gray-950 text-white pb-20 pt-20">
            ...
            <div className="relative w-full aspect-video bg-black group">
                {/* 
                  If we have a videoUrl, we try to embed it. 
                  Youtube util handles standard youtube links.
                */}
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-6 py-8 md:py-12">
                <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8 md:gap-12">
                    ...
                    <div className="hidden md:block relative aspect-[2/3] rounded-2xl overflow-hidden shadow-2xl shadow-indigo-500/10">
                        {/* Using img tag to be safe if next.config removed domains */}
                        <img
                            src={movie.imageUrl}
                            alt={movie.title}
